package com.lv.components.scrollbars {
	 ...
	 * @author kafeichong
	 */
	public class lvFixedAngleScrollBar extends MovieClip {
		private var _scrollbar : lvAngleScrollbar;
		// ==========================================================================
		// STAGE INSTANCES
		// ==========================================================================
		// 上按钮
		...
		public function lvFixedAngleScrollBar() {
		...
		private function _initialize(e : Event) : void {
			...
			_scrollbar = new lvAngleScrollbar(stage);
			// 可以通过属性传递
			...
			lowerBound,// 最小值 
			angle);
			// _scrollbar.setup(content, key, contentSize, maskSize, upperBound, lowerBound, angle)
			// 通过属性更改值
			...
			// _scrollbar.addEventListener(JPPScrollbarEvent.SCROLL_START   , function(e:JPPScrollbarEvent):void { trace("scroll start");    } );
			 _scrollbar.addEventListener(lvScrollbarEvent.SCROLL_PROGRESS, function(e:lvScrollbarEvent):void {
				 trace(_scrollbar.property,target.y);
				 target.x=-target.y*lvMath.toRadian(angle)
				 trace("scroll progress"); } );
