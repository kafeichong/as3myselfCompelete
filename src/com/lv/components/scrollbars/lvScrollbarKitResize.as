package com.lv.components.scrollbars {
	...
	public class lvScrollbarKitResize extends MovieClip {
		// ==========================================================================
		// VARIABLES
		// ==========================================================================
		/**
		...
		// CONSTRUCTOR
		// ==========================================================================
		public function lvScrollbarKitResize() : void {
		...
		private function _initialize(e : Event) : void {
			...
			// 上向きアローボタンとしてバインドするインスタンスを設定します.
			// _scrollbar.down = arrowDown;
			// 下向きアローボタンとしてバインドするインスタンスを設定します.
			_scrollbar.base = base;
			// スクロールエリアとしてバインドするインスタンスを設定します.
			_scrollbar.slider = slider;
			// スライダとしてバインドするインスタンスを設定します.

			// ======================================================================
			// スクロールバーの初期化
			_scrollbar.setup(target,      // スクロール対象となるオブジェクトです． 
			...
			// setup関数でstageを渡さない場合はJPPScrollbarのstageプロパティへstageを代入する必要があります.
			// _scrollbar.stage = stage;
			...
			_scrollbar.startAutoScroll(false);
			...
			// スクロール完了時に呼び出されます．
			// _scrollbar.addEventListener(JPPScrollbarEvent.SCROLL_COMPLETE, function(e:JPPScrollbarEvent):void { 		transitionInComplete();
		}

		private function removeStage(event : Event) : void {
		...
		private function resizeHandler(event : Event) : void {
			...
			var slider : MovieClip = scrollBox.slider;
			var base : MovieClip = scrollBox.base;
			// mainBg.height = appHeight - 100 - 120;
			mainBg.height = appHeight - 100;
			mask.height = mainBg.height - 30;
			base.height = mainBg.height - 30;
			body.y = 0;
			...
			if (_scrollbar) {
				...
				if (_scrollbar.isUnderFlow) {
					trace("_scrollbar.isUnderFlow:" + _scrollbar.isUnderFlow)
					...
					trace("_scrollbar.isUnderFlow:" + _scrollbar.isUnderFlow)
					_scrollbar.slider.visible = true;
					_scrollbar.base.visible = true;
